...
CREATE TABLE IF NOT EXISTS t_schedules_cron (
    ...
    ignore_misfire   BOOLEAN NOT NULL,
    is_active        BOOLEAN NOT NULL,
    created_at       TIMESTAMP(6) WITH TIME ZONE NOT NULL,
    last_executed_at TIMESTAMP(6) WITH TIME ZONE,
    execution_status VARCHAR(10) NOT NULL CHECK (execution_status IN ('ACTIVE', 'TERMINATED')),
    execution_note   JSON,
    PRIMARY KEY (id),
    UNIQUE (reference_id, origin)
);

CREATE TABLE IF NOT EXISTS t_schedules_onetime (
    id               BIGINT GENERATED BY DEFAULT AS IDENTITY,
    origin           VARCHAR(255) NOT NULL,
    reference_id     VARCHAR(255),
    group_id         VARCHAR(255),
    ...
    execution_status VARCHAR(9) NOT NULL CHECK (execution_status IN ('SCHEDULED', 'EXECUTED', 'CANCELLED')),
    execution_note   JSON,
    PRIMARY KEY (id),
    UNIQUE (reference_id, origin)
);
